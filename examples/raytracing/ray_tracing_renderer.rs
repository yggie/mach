...
pub struct RayTracingRenderer<T: RayTracer> {
    canvas: Canvas,
    sequence: Sequence,
    ray_tracer: T,
...
impl<T> RayTracingRenderer<T> where T: RayTracer {
    pub fn new_with_file_output(canvas: Canvas, ray_tracer: T, filename: &str) -> RayTracingRenderer<T> {
